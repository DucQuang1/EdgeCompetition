...
HeadlineWordsRF = randomForest(factor(Popular) ~ ., data=TrainHeadline)
PredTest = as.data.frame(predict(HeadlineWordsRF, newdata=TestHeadline, type = "prob"))

outcome = "Popular"
modelCasualglm$results
Predgbm = predict(modelCasualglm, testSet, type = "prob")
